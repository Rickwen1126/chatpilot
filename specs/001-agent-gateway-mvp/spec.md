...
系統根據路由規則（群組 ID 對應、關鍵字比對）將訊息
自動分派至正確的 agent。未匹配的訊息靜默忽略，
避免不必要的 AI 成本和使用者干擾。
...
4. **Given** 已路由群組但訊息未命中任何關鍵字且無預設 agent，
   **When** 使用者發送「今天天氣好」，
   **Then** 系統不回應
...
- **FR-001**: 系統 MUST 定義統一訊息格式（`Message`），包含
  文字內容、使用者識別、來源頻道、群組識別、平台 context
...
- **FR-005**: 系統 MUST 實作路由分派器（dispatcher），
  依序以群組 ID → 關鍵字 → 預設處理進行訊息路由
- **FR-006**: 系統 MUST 整合 Copilot SDK 作為 agent 執行引擎，
  支援 planning、tool calling、multi-turn 對話能力
...
- **Message**: 統一輸入格式——文字內容、使用者 ID、
  來源頻道標識、群組 ID（可為空）、平台 context（如
  reply token 等平台特有資料）
- **Response**: 統一輸出格式——文字內容、附件清單
  （MVP 僅文字）
- **Channel Adapter**: 頻道轉接器——負責平台協議轉換，
  實作統一介面即可接入系統。每個平台一個 adapter
- **Dispatcher**: 路由分派器——根據路由規則將 `Message`
  導向正確的 agent。包含群組路由表與關鍵字路由表
- **Agent**: AI 代理——由 Copilot SDK 驅動，擁有自己的
  prompt 與 tool 定義。接收 `Message`、回傳 `Response`
- **Route Map**: 路由表——定義群組 ID → agent 名稱、
  關鍵字 → agent 名稱的對應關係
...
- cloudflared tunnel 已設定完成，提供外部 HTTPS 入口
  供 LINE webhook 使用
- MVP 階段 agent 不需要跨 session 對話記憶
