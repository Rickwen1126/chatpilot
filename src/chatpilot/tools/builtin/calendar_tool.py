...
def create_calendar_tool() -> ToolDefinition:
    ...
    async def handler(invocation: ToolInvocation) -> ToolResult:
        ...
        for i in range(7):
            ...
            week_lines.append(
                f"  {WEEKDAY_MAP[i]} = {d.strftime('%m/%d')}{mark}"
            )
        ...
        for i in range(7):
            ...
            next_lines.append(
                f"  {WEEKDAY_MAP[i]} = {d.strftime('%m/%d')}"
            )
        ...
        result = (
            ...
            f"（{WEEKDAY_MAP[(today + timedelta(1)).weekday()]}）"
            f"\n後天 = {(today + timedelta(2)).strftime('%m/%d')}"
            f"（{WEEKDAY_MAP[(today + timedelta(2)).weekday()]}）"
        )
        return ToolResult(
